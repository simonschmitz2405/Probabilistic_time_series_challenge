...
from processing.energy_data_importer import EnergyDataImporter
import pandas as pd
import holidays
...
class DataPreparing:
    """Class to prepare the data for the models.

    Attributes:
        None

    Functions:
        get_bike_and_weather_data: Get the bike data and the weather data as a single dataframe.
        get_bike_data: Call the BikeDataImporter to get the bike data.
        get_energy_and_weather_data: Get the combined data with energy and weather data on hourly basis.
        get_energy_data: Get the energy data from the EnergyDataImporter.
        get_weather_data_hourly: Get the weather data for Karlsruhe from the historical and forecast data on hourly basis.
        get_weather_data_daily: Resample the hourly weather data to daily data.
        create_features: Create the features based on the data.
        create_features_iterative: Create the features based on the data iteratively.
        combine_data_weather: Merge the bike data with the weather data.
    """
    ########################################
    ## Bike Data ###########################
    ########################################
    ...
    ## Energy Data ###########################
    ########################################
    def get_energy_and_weather_data(self):
        """Get the combined data with energy and weather data on hourly basis.

        Returns:
            data_w_weather (pd.Dataframe): The energy data with the weather data
        """
        energy_data = self.get_energy_data()

        weather_data_hourly = self.get_weather_data_hourly_germany()
        data_w_weather = self.combine_data_weather(energy_data, weather_data_hourly)

        return data_w_weather

    
    def get_energy_data(self):
        """Get the energy data from the EnergyDataImporter.

        Returns:
            data (pd.Dataframe): Energy data on hourly basis.
        """
        energy_data_importer = EnergyDataImporter()
        data = energy_data_importer.get_energy_data()
        return data
    ...
    def get_weather_data_hourly_germany(self):
        """Get the weather data for Germany from the historical and forecast data on hourly basis.
        ...
        history_data_path = (
            ...
            / "history"
            / "germany"
            / f"history_germany_{todays_date}.csv"
        )
        forecast_data_path = (
            path
            / "data"
            ...
            / "forecasts"
            / "germany"
            / f"forecast_germany_{todays_date}.csv"
        )
    ...
    ## Feature Creation #####################
    ########################################
    def create_features(self, data, TARGET):
        """Create the features based on the data.

        Args:
            data (pd.Dataframe): The data to create the features for.
            TARGET (str): The target column to create the features for.

        Returns:
            data (pd.Dataframe): Data with the created features.
        """
        data = data.copy()
        ...
        # Create lag features
        target_map = data[TARGET].to_dict()
        ...
        data["rolling_mean_30"] = data[TARGET].shift(1).rolling(window=30).mean()
        data["rolling_std_7"] = data[TARGET].shift(1).rolling(window=7).std()
        data["rolling_std_30"] = data[TARGET].shift(1).rolling(window=30).std()

        data["diff_prev_day"] = data[TARGET].shift(1).diff()

        return data

    def create_features_iterative(self, data, TARGET):
        ...
        for lag in [1, 2, 3, 4]:
            data[f"diff_lag_{lag}"] = data[TARGET].shift(1).diff(lag)
    ...
    # Utility function
    def combine_data_weather(self, data, weather_data):
        ...
        """
        data_and_weather_data = pd.merge(
            data, weather_data, left_index=True, right_index=True, how="outer"
        )
        return data_and_weather_data
